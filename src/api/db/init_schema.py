...
async def init_db():
    ...
    # Set defaults only if the DB file is brand new
    if not os.path.exists(sqlite_db_path):
        set_db_defaults()

    async with get_new_db_connection() as conn:
        cursor = await conn.cursor()
        if not await check_table_exists('organizations', cursor): await create_organizations_table(cursor)
        if not await check_table_exists('org_api_keys', cursor): await create_org_api_keys_table(cursor)
        if not await check_table_exists('users', cursor): await create_users_table(cursor)
        if not await check_table_exists('user_organizations', cursor): await create_user_organizations_table(cursor)
        if not await check_table_exists('milestones', cursor): await create_milestones_table(cursor)
        if not await check_table_exists('cohorts', cursor): await create_cohort_tables(cursor)
        if not await check_table_exists('courses', cursor): await create_courses_table(cursor)
        if not await check_table_exists('course_cohorts', cursor): await create_course_cohorts_table(cursor)
        if not await check_table_exists('tasks', cursor): await create_tasks_table(cursor)
        if not await check_table_exists('questions', cursor): await create_questions_table(cursor)
        if not await check_table_exists('scorecards', cursor): await create_scorecards_table(cursor)
        if not await check_table_exists('question_scorecards', cursor): await create_question_scorecards_table(cursor)
        if not await check_table_exists('chat_history', cursor): await create_chat_history_table(cursor)
        if not await check_table_exists('task_completion', cursor): await create_task_completion_table(cursor)
        if not await check_table_exists('course_tasks', cursor): await create_course_tasks_table(cursor)
        if not await check_table_exists('course_milestones', cursor): await create_course_milestones_table(cursor)
        if not await check_table_exists('course_generation_jobs', cursor): await create_course_generation_jobs_table(cursor)
        if not await check_table_exists('task_generation_jobs', cursor): await create_task_generation_jobs_table(cursor)
        if not await check_table_exists('code_drafts_table_name', cursor): await create_code_drafts_table(cursor)
